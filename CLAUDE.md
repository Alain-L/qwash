...
- **`analysis/`**: Bloat detection and estimation
  - `types.go`: Core data structures (`BloatTable`, `BloatIndex`)
...
- **`sql/`**: Embedded SQL queries
  ...
  - `table_bloat.sql`: ioguix-based table bloat estimation query
  - `btree_bloat.sql`: B-Tree index bloat estimation query
...
- **`testdata/`**: Test data generators and benchmarking scripts
  ...
  - `POC.sql`: Proof of concept SQL scripts
...
- CLI framework (Cobra)
- Basic bloat reduction algorithm (RunQwash + CompactTable)
- Bloat page estimation (GetBloatPages)
- Text and JSON output formatters

**TODO** (see TODO.md):
- Full bloat estimation implementation (--estimate flag shows "[TODO]" messages)
