...
// CompactTableUpdateFast is a fast variant of the UPDATE method.
...
// This is faster but may require multiple passes for complete compaction.
// Use --update --fast for this mode.
func (db *DB) CompactTableUpdateFast(tableName string) error {
	ctx := context.Background()
...
// CompactTableUpdate performs table compaction using the UPDATE method with rounds.
// This implements the pgcompacttable algorithm:
// - Round 1: Process pages from end down to (page_count - bloat_pages)
// - After VACUUM: if table shrunk, set toPage = newPageCount - 1 and start new round
// - Repeat until table stops shrinking
// For faster single-pass compaction, use --update --fast.
func (db *DB) CompactTableUpdate(tableName string) error {
	...
	// Calculate initial to_page using bloat estimation (like pgcompacttable first round)
	bloatPages, err := db.GetBloatPages(tableName)
	if err != nil {
		return fmt.Errorf("failed to estimate bloat: %w", err)
	}
	...
	// pgcompacttable parameters
	pagesPerRound := 1
	maxTuplesPerPage := 226

	// Prepare table and column identifiers for the procedure
	tableIdent := sanitizeTableName(tableName)
	columnIdent := pgx.Identifier{column}.Sanitize()

	// ROUNDS LOOP (like pgcompacttable)
	// Each round processes pages from currentPage down to toPage
	// After each round, if table shrunk, start new round with toPage = newPageCount - 1
	totalPagesProcessed := 0
	roundNumber := 0
	pageCount := initialPages

	for pageCount > toPage {
		roundNumber++
		pagesBeforeVacuum := pageCount / 16
		if pagesBeforeVacuum < 1 {
			pagesBeforeVacuum = 1
		}

		if db.Verbose {
			fmt.Printf("  Round %d: pages %d -> %d\n", roundNumber, pageCount-1, toPage)
		}

		// Process pages in this round
		currentPage := pageCount - 1
		pagesSinceVacuum := 0
		roundPagesProcessed := 0
		...
		for currentPage > toPage {
			...
			roundPagesProcessed += pagesPerRound
			pagesSinceVacuum += pagesPerRound
			currentPage -= pagesPerRound

			// Progress display
			if !db.SilentProgress {
				fmt.Printf("\r\033[K  Round %d: page %d/%d | vacuum in %d",
					roundNumber, currentPage, toPage, pagesBeforeVacuum-pagesSinceVacuum)
			}

			// VACUUM every N pages
			if pagesSinceVacuum >= pagesBeforeVacuum {
				_, err = db.conn.Exec(ctx, fmt.Sprintf("VACUUM %s", sanitizeTableName(tableName)))
				if err != nil {
					return fmt.Errorf("VACUUM failed at page %d: %w", currentPage, err)
				...
				actualPages, _ := db.GetTablePages(tableName)
				if db.Verbose {
					fmt.Printf("\r\033[K  VACUUM at page %d, current size: %d pages\n",
						currentPage, actualPages)
				}

				// If we've gone past the actual table size, end this round
		...
		totalPagesProcessed += roundPagesProcessed
		...
		// End of round: VACUUM and check if table shrunk
		_, err = db.conn.Exec(ctx, fmt.Sprintf("VACUUM %s", sanitizeTableName(tableName)))
		if err != nil {
			return fmt.Errorf("VACUUM failed after round %d: %w", roundNumber, err)
		}

		newPageCount, _ := db.GetTablePages(tableName)
		if db.Verbose {
			fmt.Printf("  Round %d complete: %d -> %d pages\n", roundNumber, pageCount, newPageCount)
		}

		// If table didn't shrink, we're done
		if newPageCount >= pageCount {
			break
		}

		// Table shrunk: prepare next round
		// pgcompacttable sets toPage = pageCount - 1 for subsequent rounds
	...
	finalPages, _ := db.GetTablePages(tableName)
	if db.Verbose {
		fmt.Printf("  Done: %d -> %d pages (%d rounds, %d pages processed)\n",
			initialPages, finalPages, roundNumber, totalPagesProcessed)
	}
