...
// CompactTableUpdate performs table compaction using the UPDATE method (like pgcompacttable).
// This is a direct port of pgcompacttable's algorithm:
// - Creates a stored procedure for UPDATE SET col = col
// - Processes pages from the end, one at a time
// - VACUUMs every N pages (N = total_pages / 16)
// - Loops until max_page <= to_page (target minimum pages)
